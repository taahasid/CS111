    ...
    int month, double[][] drywet, double[][] wetwet) {
    	...
    	for (int i = 0; i < drywet.length; i++)
    		if (longitute == drywet[i][0] && latitude == drywet[i][1])
    			valueOfDry = drywet[i][month];
        
        
        for (int i = 0; i < wetwet.length; i++)
    		if (longitute == wetwet[i][0] && latitude == wetwet[i][1])
    			valueOfWet = wetwet[i][month];
    		...
    		if (array[i-1] == WET) {
    			if (StdRandom.uniform() <= valueOfWet)
    				array[i] = WET;
    			else
    				array[i] = DRY;
    		}
    		else {
    			if (StdRandom.uniform() <= valueOfDry)
    				array[i] = WET;
    			else
    				array[i] = DRY;
    		}
